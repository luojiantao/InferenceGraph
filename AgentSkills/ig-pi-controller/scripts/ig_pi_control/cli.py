...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="InferenceGraph Pi control plane")
    ...
    pi_subagent = subparsers.add_parser("build-pi-subagent-call")
    pi_subagent.add_argument("--action", required=True)
    pi_subagent.add_argument("--cwd", required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "build-pi-subagent-call":
            _write_json(build_pi_subagent_call(_read_json(args.action), cwd=args.cwd))
            return 0
